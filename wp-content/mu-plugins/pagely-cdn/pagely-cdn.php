...
function pagely_cdn_options()
{
	...
	$example_cdn_uri = 'http://s####.pcdn.co';
	$example_https_cdn_uri = 'https://s####.pcdn.co';
...
function pagely_purge_cdn()
{
	...
	$alert = Pagely_Alert::instance();
	if ($result === null) {
		$alert->setAlert('CDN Purge: Error decoding API response, please contact Pagely Support.', false);
	} elseif (isset($result->error)) {
		$alert->setAlert('CDN Purge: '.$result->error, false);
	} elseif ($result->result == 3) {
		$alert->setAlert('CDN Purge: '.$result->message, false);
	} else {
		$alert->setAlert('CDN Purge: Success (will take 3 to 5 minutes to complete)');
	}
}
